...
- **自動削除**: ナレッジエントリ削除時にベクトルも自動削除
- **RAG検索**: ベクトル類似度検索による高精度な知識検索
...
- **削除機能**: 不要なFAQの削除機能
- **RAG検索**: ベクトル類似度検索による高精度な知識検索と回答生成
...
### v3.2.0 - RAG検索最適化 ✅ **NEW!**
- ✅ **RAG検索精度向上**: DynamoDBキー構造の修正による正確なナレッジコンテキスト取得
- ✅ **パフォーマンス最適化**: 29秒API制限内での高速処理（2-10秒）
...
### v3.1.0 - ナレッジ削除機能の最適化 ✅
- ✅ **ベクトル自動削除**: ナレッジエントリ削除時にベクトルも自動削除
- ✅ **IAM権限最適化**: `BatchWriteItem`権限を追加して削除機能を完全対応
- ✅ **バッチ削除処理**: 効率的なベクトル一括削除機能
- ✅ **エラーハンドリング**: 削除失敗時の詳細ログ出力
- ✅ **Terraform同期**: IAMポリシー更新とTerraform状態の完全同期
- ✅ **データ整合性**: ナレッジとベクトルの一貫性を保証

### v3.0.0 - ファイルアップロード機能の改善 ✅
...
### 🗑️ ナレッジ削除機能の最適化
- **ベクトル自動削除**: ナレッジエントリ削除時にベクトルも自動削除
- **バッチ削除処理**: `BatchWriteItem`による効率的な一括削除
- **IAM権限最適化**: 削除機能に必要な権限を完全対応
- **データ整合性**: ナレッジとベクトルの一貫性を保証

### 🔧 技術的改善
- **エラーハンドリング**: 削除失敗時の詳細ログ出力
- **Terraform同期**: IAMポリシー更新とTerraform状態の完全同期
- **権限管理**: 最小権限の原則に基づくIAM設定
