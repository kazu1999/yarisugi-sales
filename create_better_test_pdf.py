import io
import base64

def create_better_test_pdf():
    """より完全なテスト用PDFファイルを作成"""
    
    # より完全なPDFファイルのBase64データ（EOF markerを含む）
    pdf_base64 = ''''''
    
    # Base64デコードしてPDFファイルを作成
    pdf_bytes = base64.b64decode(pdf_base64)
    
    # ファイルに保存
    with open('Yarisugi基礎-顧客個別画面_完全版.pdf', 'wb') as f:
        f.write(pdf_bytes)
    
    print('✅ より完全なテスト用PDFファイルが作成されました: Yarisugi基礎-顧客個別画面_完全版.pdf')
    print(f'📄 ファイルサイズ: {len(pdf_bytes)} bytes')
    print('📋 このファイルは日本語ファイル名を含むdata URLのテストに使用できます')
    
    return 'Yarisugi基礎-顧客個別画面_完全版.pdf'

if __name__ == '__main__':
    create_better_test_pdf()
